...
import { Ap, CtoTxAverage, GponIntegration } from "../../../_lib/database/main";
...
class CtoTxAverageController {
    async index(req: Request, res: Response): Promise<Response<CtoGraphDTO[]>> {
        try {
            ...
            const ctoCount = await CtoTxAverage.findAll({
                where: {
                    gpon_integration_id: {
...
export default new CtoTxAverageController();
