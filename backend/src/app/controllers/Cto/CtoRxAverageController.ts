...
import { Ap, CtoRxAverage, GponIntegration } from "../../../_lib/database/main";
...
class CtoRxAverageController {
    async index(req: Request, res: Response): Promise<Response<CtoGraphDTO[]>> {
        try {
            ...
            const ctoCount = await CtoRxAverage.findAll({
                where: {
                    gpon_integration_id: {
...
export default new CtoRxAverageController();
